##########################################
##----------------ONI-------------------##
##########################################

image oni neutral = im.FactorScale("sprites/oni/Oni_Neutral.png", 0.30, bilinear=True)
image oni neutral_smile = im.FactorScale("sprites/oni/Oni_Neutral_Smile.png", 0.30, bilinear=True)
image oni neutral_smile_eye = im.FactorScale("sprites/oni/Oni_Neutral_Smile_Eye.png", 0.30, bilinear=True)
image oni angry = im.FactorScale("sprites/oni/Oni_Angry.png", 0.30, bilinear=True)
image oni angry_eye = im.FactorScale("sprites/oni/Oni_Angry_Eye.png", 0.30, bilinear=True)
image oni shock = im.FactorScale("sprites/oni/Oni_Shock.png", 0.30, bilinear=True)
image oni shock_eye = im.FactorScale("sprites/oni/Oni_Shock_Eye.png", 0.30, bilinear=True)
image oni shock2 = im.FactorScale("sprites/oni/Oni_Shock2.png", 0.30, bilinear=True)
image oni shock2_eye = im.FactorScale("sprites/oni/Oni_Shock2_Eye.png", 0.30, bilinear=True)
image oni shriek = im.FactorScale("sprites/oni/Oni_Shriek.png", 0.30, bilinear=True)
image oni shriek_eye = im.FactorScale("sprites/oni/Oni_Shriek_Eye.png", 0.30, bilinear=True)
image oni smile = im.FactorScale("sprites/oni/Oni_Smile.png", 0.30, bilinear=True)
image oni yell = im.FactorScale("sprites/oni/Oni_Yell.png", 0.30, bilinear=True)
image oni yell_eye = im.FactorScale("sprites/oni/Oni_Yell_Eye.png", 0.30, bilinear=True)

##########################################
##-----------------UTAGU----------------##
##########################################

image utagu smile = im.FactorScale("sprites/utagu/Utagu_Smile.png", 0.30, bilinear=True)
image utagu cry = im.FactorScale("sprites/utagu/Utagu_Cry.png", 0.30, bilinear=True)
image utagu cry2 = im.FactorScale("sprites/utagu/Utagu_Cry2.png", 0.30, bilinear=True)
image utagu despair = im.FactorScale("sprites/utagu/Utagu_Despair.png", 0.30, bilinear=True)
image utagu frown = im.FactorScale("sprites/utagu/Utagu_Frown.png", 0.30, bilinear=True)
image utagu huh = im.FactorScale("sprites/utagu/Utagu_Huh.png", 0.30, bilinear=True)
image utagu uuh = im.FactorScale("sprites/utagu/Utagu_Uuh.png", 0.30, bilinear=True)
image utagu worry = im.FactorScale("sprites/utagu/Utagu_Worry.png", 0.30, bilinear=True)
image utagu worry2 = im.FactorScale("sprites/utagu/Utagu_Worry2.png", 0.30, bilinear=True)

##########################################
##-----------------KOKORO---------------##
##########################################

image kokoro neutral = im.FactorScale("sprites/kokoro/Kokoro_Neutral.png", 0.30, bilinear=True)
image kokoro smile = im.FactorScale("sprites/kokoro/Kokoro_Smile.png", 0.30, bilinear=True)
image kokoro smile_open = im.FactorScale("sprites/kokoro/Kokoro_Smile_Open.png", 0.30, bilinear=True)
image kokoro grin = im.FactorScale("sprites/kokoro/Kokoro_Grin.png", 0.30, bilinear=True)
image kokoro grin2 = im.FactorScale("sprites/kokoro/Kokoro_Grin2.png", 0.30, bilinear=True)
image kokoro laugh = im.FactorScale("sprites/kokoro/Kokoro_Laugh.png", 0.30, bilinear=True)
image kokoro shock1 = im.FactorScale("sprites/kokoro/Kokoro_Shock.png", 0.30, bilinear=True)
image kokoro shock2 = im.FactorScale("sprites/kokoro/Kokoro_Shock2.png", 0.30, bilinear=True)
image kokoro frown = im.FactorScale("sprites/kokoro/Kokoro_Frown.png", 0.30, bilinear=True)
image kokoro worry = im.FactorScale("sprites/kokoro/Kokoro_Worry.png", 0.30, bilinear=True)

##########################################
##-------------------KURA---------------##
##########################################

image kura smile = im.FactorScale("sprites/kura/Kura_Smile.png", 0.30, bilinear=True)
image kura hmm = im.FactorScale("sprites/kura/Kura_Hmm.png", 0.30, bilinear=True)
image kura frown = im.FactorScale("sprites/kura/Kura_Frown.png", 0.30, bilinear=True)
image kura mad = im.FactorScale("sprites/kura/Kura_Mad.png", 0.30, bilinear=True)
image kura cry_sad = im.FactorScale("sprites/kura/Kura_Cry_Sad.png", 0.30, bilinear=True)
image kura cry_mad = im.FactorScale("sprites/kura/Kura_Cry_Mad.png", 0.30, bilinear=True)
image kura cry_frown = im.FactorScale("sprites/kura/Kura_Cry_Frown.png", 0.30, bilinear=True)
image kura cunning = im.FactorScale("sprites/kura/Kura_Cunning.png", 0.30, bilinear=True)
image kura cunning2 = im.FactorScale("sprites/kura/Kura_Cunning2.png", 0.30, bilinear=True)
image kura end = im.FactorScale("sprites/kura/Kura_End.png", 0.30, bilinear=True)
image kura end2 = im.FactorScale("sprites/kura/Kura_End2.png", 0.30, bilinear=True)

